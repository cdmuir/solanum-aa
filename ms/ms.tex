...
The raw data set consisted of 2400 \agcurve{} curves with an average of
100. Principled, consistent, manual curation of a data set this size is
not feasible. Therefore, we automated data cleaning using custom
\emph{R} scripts. Cleaning is divided into XXX sequential steps:
...
\item
  remove replicates with no overlap between amphi and pseudohypo
  \agcurve{} curves
\item
  Trim extreme \aax{} values
\end{enumerate}

\textbf{Table showing number of curves and points before and after each
cleaning step}

\paragraph{Remove unreliable data
points}\label{remove-unreliable-data-points}
...
studentized residual greater than until none remained.
...
\emph{Procedure}: We privionsally estimated \aax{} for each replicate by
...
\item
  ...
  for each parameter was greater than \(10^3\).
\end{itemize}
