...
photo_2d_pm = function(stomatal_distribution, replace = NULL, ...) {
  
  # Set parameters
  parms = set_2d_pm_parms(replace) |>
    derive_2d_pm_parms(fit_rubisco = fit_rubisco) |>
    make_2d_pm_mat() |>
    add_stomatal_position(stomatal_distribution)
...
add_stomatal_position = function(parms, stomatal_distribution) {
  
  parms[["x_abaxial_stomata"]] = switch(
...
get_2d_pm_default_parms = function(...) {
  ...
  read_csv("data/2d-pm-parameters.csv", col_types = "cccdccl") |>
    filter(!calculated) |>
