...
df_stomata_new = crossing(
  acc = unique(d1$acc),
  light_treatment = c("low", "high")
) |>
  mutate(row = row_number())

df_stomata_pred = full_join(
  posterior_epred(fit_stomata, newdata = df_stomata_new, resp = "lowersd") |>
    t() |>
    as_draws_df() |>
    mutate(row = row_number()) |>
    pivot_longer(
  ...
  posterior_epred(fit_stomata, newdata = df_stomata_new, resp = "uppersd") |>
    t() |>
    as_draws_df() |>
    mutate(row = row_number()) |>
    pivot_longer(
    ...
    full_join(df_stomata_new, by = "row"),
  by = join_by(row, draw, acc, light_treatment)
) |>
  mutate(sr = exp(upper_sd) / (exp(lower_sd) + exp(upper_sd))) |>
...
ggplot(df_stomata_pred, aes(Growth, sr, group = acc, color = Growth)) +
