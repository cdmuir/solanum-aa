...
r = list(I_0 = 0.0001, n_x = 53, n_z = 201, phi_pal = 0.1)
ph2d_amphi = photo_2d_pm("amphi", fit_rubisco = fit_rubisco, replace = r)
ph2d_hypo = photo_2d_pm("hypo", fit_rubisco = fit_rubisco, replace = r)

# Plot results
df_C = expand.grid(
  z = seq_len(ph2d_amphi$parms[["n_z"]]),
  x = seq_len(ph2d_amphi$parms[["n_x"]]),
  name = c("italic(C)[ias]", "italic(C)[liq]"),
  distribution = c("amphi", "hypo")
) |>
  mutate(value = c(ph2d_amphi$fit$y, ph2d_hypo$fit$y))
...
df_points = crossing(
  ...
  mutate(x = 0 + (z > 0 & distribution == "amphi") * r$n_x, value = numeric(8))

ggplot(df_C, aes(x, z)) +
  facet_grid(distribution ~ name, labeller = label_parsed) +
