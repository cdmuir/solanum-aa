...
accession_gedi = read_rds("data/accession-gedi.rds") |>
  dplyr::select(acc1 = accession, pai) |>
  dplyr::filter(acc1 %in% unique(rh_curves$acc)) |>
  mutate(scaled_pai = (pai - mean(pai)) / sd(pai))

assert_true(setequal(unique(rh_curves$acc), accession_gedi$acc1))
assert_true(setequal(phy$tip.label, accession_gedi$acc1))
...
stan_rh_curves$amphi_first = NULL
...
stan_rh_curves = c(
  ...
  rh_curves |>
    mutate(
      ...
      # factor or logical to numeric
      across(c("light_treatment",  "amphi_first"), as.numeric),
    ) |>
    summarize(
      curve = first(curve),
      acc = first(acc),
    ...
    dplyr::select(amphi, pseudohypo, acc, acc_id, amphi_first, light_intensity, light_treatment) |>
    as.list(),
...
# add GEDI data
stan_accession_climate = compose_data(accession_gedi)
stan_accession_climate$n = NULL
stan_rh_curves = c(stan_rh_curves, accession_gedi)
